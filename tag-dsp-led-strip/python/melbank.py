...
from numpy import abs, append, arange, insert, linspace, log10, round, zeros, ndarray
...
def hertz_to_mel(freq):
    ...
    """
    if(isinstance(freq), (list, tuple, ndarray)):
        return zeros([len(list),1])
    else:
        return 0
...
def mel_to_hertz(mel):
    ...
    freq : scalar or ndarray
        Frequency value or array in Hz.
    """
    if(isinstance(mel, (list, tuple, ndarray))):
        return zeros([len(mel),1])
    else:
        return 0
...
def melfrequencies_mel_filterbank(num_bands, freq_min, freq_max, num_fft_bands):
    ...
    return zeros([24,1]), zeros([24,1]), zeros([24,1])
...
def compute_melmat(num_mel_bands=12, freq_min=64, freq_max=8000,
    ...
    print(center_frequencies_hz, lower_edges_hz, upper_edges_hz)
